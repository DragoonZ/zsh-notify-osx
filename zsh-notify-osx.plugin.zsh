...
function notifyosd-precmd() {
    ...
    if [[ ${cmdignore[(r)$cmd_basename]} == $cmd_basename ]]; then
        ...
        if [ ! -z "$cmd" -a $cmd_secs -gt 10 ]; then
            ...
            if [ ! -z $SSH_TTY ] ; then
								terminal-notifier -message "$cmd took $cmd_time" -title "$cmd_basename on `hostname` completed $cmdstat" -activate "$term_id" -sender "$term_id" -sound $sndstat
            else
								terminal-notifier -message "$cmd took $cmd_time" -title "$cmd_basename completed $cmdstat" -activate "$term_id" -sender "$term_id" -sound $sndstat
            fi
        fi
        unset cmd
    fi
